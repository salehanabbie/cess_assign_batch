 ...
 *
 */
public class DelDeliveryProvider extends SortableDataProvider{
...
{  
	...
	public DelDeliveryProvider()
	{
		...
		opsFileRegList = (List<OpsFileRegModel>) controller.viewAllDelDelivery();
	...
	public DelDeliveryProvider(String fileName)
	{
		log.debug("In the provider by criteria constructor");
		setSort("fileName", SortOrder.ASCENDING);
		opsFileRegList = (List<OpsFileRegModel>) controller.viewDelDeliveryByCriteria(fileName);
		webOpsFileRegList = new ArrayList<WebOpsFileRegModel>();
		if(opsFileRegList.size() > 0)
		{
			for (OpsFileRegModel fileRegCommonModel : opsFileRegList) 
