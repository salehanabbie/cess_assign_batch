...
import com.bsva.provider.ConfgErrorCodesProvider;
 ...
 *
 */
public class ViewConfgErrorCodesPanel extends Panel 
{
	Controller controller;
	private static final long serialVersionUID = 1L;
	public static Logger log = Logger.getLogger(ViewConfgErrorCodesPanel.class);
	public static String id;
	private Form form;
	private Set<WebConfgErrorCodesModel> selected = new HashSet<WebConfgErrorCodesModel>();
	private Button updateButton;
	private Button addButton;
	private Button searchButton;
	private Button viewAllButton;
	private TextField<String> searchText;
	MaintainConfgErrorCodesPanel maintainConfgErrorCodesPanel;
	private ClientSessionModel clientSessionModel;
    public static Session session;
    private String userRole;

	public ViewConfgErrorCodesPanel(String id) 
	{
		super(id);
		this.id = id;
	...
	private void initializeComponents() 
	{
		...
		form = new Form("viewConfgErrorCodesPanel");
		form.add(createDataTable(new ConfgErrorCodesProvider()));
		updateButton = new Button("updateButton") 
		{
			...
			public void onSubmit() 
			{
				...
				{
					for (WebConfgErrorCodesModel webErrorCodesModel : selected) 
					{
						maintainConfgErrorCodesPanel = new MaintainConfgErrorCodesPanel(id, webErrorCodesModel, "update");
						MarkupContainer markupContainer = form.getParent().getParent();
						markupContainer.remove(form.getParent());
						markupContainer.add(maintainConfgErrorCodesPanel);
						maintainConfgErrorCodesPanel.setOutputMarkupId(true);
						maintainConfgErrorCodesPanel.setOutputMarkupPlaceholderTag(true);
						TemplatePage.setContent(maintainConfgErrorCodesPanel);
						break;
					}
				}
			}
		...
		addButton = new Button("addButton") 
		{
			...
			public void onSubmit() 
			{
				maintainConfgErrorCodesPanel = new MaintainConfgErrorCodesPanel(id, "create");
				MarkupContainer markupContainer = form.getParent().getParent();
				markupContainer.remove(form.getParent());
				markupContainer.add(maintainConfgErrorCodesPanel);
				maintainConfgErrorCodesPanel.setOutputMarkupId(true);
		...
		searchButton = new Button("searchButton") 
		{
			...
			public void onSubmit() 
			{
				...
				else
				{
					...
					form.add(createDataTable(new ConfgErrorCodesProvider(searchText.getValue())));
				}
			}
		};
	...
	private DefaultDataTable createDataTable(ConfgErrorCodesProvider errorCodesProvider)
	{
		DefaultDataTable dataTable;
		List<IColumn> columns = new ArrayList<IColumn>();
		IColumn column = new CheckBoxColumn<WebConfgErrorCodesModel>(Model.of("Select")) 
		{
			@Override
			protected IModel newCheckBoxModel(final IModel<WebConfgErrorCodesModel> rowModel) 
			{
		...
		columns.add(new PropertyColumn(new Model<String>("Error Code Description"), "errorDescription", "errorDescription"));
		columns.add(new PropertyColumn(new Model<String>("Active Ind"),"activeInd", "activeInd"));
		columns.add(new PropertyColumn(new Model<String>("Severity"),"severity", "severity"));
		dataTable = new DefaultDataTable("dataTable", columns,errorCodesProvider, 10);
		return dataTable;
	}
}