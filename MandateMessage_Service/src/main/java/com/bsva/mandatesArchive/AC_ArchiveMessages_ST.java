...
public class AC_ArchiveMessages_ST implements Serializable{
	...
	public AC_ArchiveMessages_ST() 
	{
		...
		{	
			log.error("AC_ArchiveMessages_ST- Could not find MandateMessageCommons.properties in classpath");
		}
	}
	...
	public void testArhiveConfHdrs() throws ParseException {
		...
		if (casSysctrlSysParamEntity.getResponsePeriod()!=null && casSysctrlSysParamEntity.getActiveInd().equalsIgnoreCase("Y") )
		{
...
//			if(delMdteResp)
//				log.info("XXXXXXXXXXXX~~~~~~MANDATE RESPONSE FOR INFO DELETED SUCCESSFULLY~~~~~XXXXXXXXXXXX");
//		}
	}
	...
	public void archiveProcessLogic() throws ParseException
	{
		...
		log.info("XXXXXXXXXXXX~~~~~~ARCHIVING MATCHED MANDATES TXNS~~~~~XXXXXXXXXXXX");
		boolean matchedTxnsBool = fileProcessBeanRemote.archiveMandateTxnsBySQL(matchType, null, archDate);
		...
		if(matchedTxnsBool)
		{
			...
			if(delMatched)
				log.info("XXXXXXXXXXXX~~~~~~MATCHED MANDATES ARCHIVED SUCCESSFULLY~~~~~XXXXXXXXXXXX");
		}

		//2. Mandate Acceptances (Pain.012)
		log.info("XXXXXXXXXXXX~~~~~~ARCHIVING MANDATE ACCEPTANCES TXNS~~~~~XXXXXXXXXXXX");
		boolean acceptedTxnsBool = fileProcessBeanRemote.archiveMandateTxnsBySQL(acceptType, null, archDate);
		...
		if(acceptedTxnsBool)
		{
			...
			if(delMatched)
				log.info("XXXXXXXXXXXX~~~~~~MANDATE ACCEPTANCES ARCHIVED SUCCESSFULLY~~~~~XXXXXXXXXXXX");
		} 

		//3. Mandate Expired 
		log.info("XXXXXXXXXXXX~~~~~~ARCHIVING EXPIRED MANDATES TXNS~~~~~XXXXXXXXXXXX");
		boolean expiredTxnsBool = fileProcessBeanRemote.archiveMandateTxnsBySQL(expireType, sdf.format(responseDate), archDate);
		log.info("expiredTxnsBool: "+expiredTxnsBool);
		if(expiredTxnsBool)
		{
			//Delete Matched Mandates 
			boolean delMatched = fileProcessBeanRemote.deleteMatchedMandateTxnsBySQL(expireType, sdf.format(responseDate));
			if(delMatched)
				log.info("XXXXXXXXXXXX~~~~~~MANDATES EXPIRED SUCCESSFULLY~~~~~XXXXXXXXXXXX");
		}

		//4. Archive Confirmation Responses
		log.info("XXXXXXXXXXXX~~~~~~ARCHIVING MANDATE CONFIRMATION ~~~~~XXXXXXXXXXXX");
		...
		if(arcConfHdrsBool && arcConfDtlsBool)
			log.info("XXXXXXXXXXXX~~~~~~OPS CONFIRMATIONS DELETED SUCCESSFULLY~~~~~XXXXXXXXXXXX");

		//		27/03/2020 - SalehaR - Optimise archive/delete Conf Hdrs/Dtls
		//		log.info("XXXXXXXXXXXX~~~~~~ARCHIVING MANDATE CONFIRMATION ~~~~~XXXXXXXXXXXX");
		//		boolean arcConfBool = beanRemote.archiveConfirmationsBySQL(archDate);
		//		if(arcConfBool)
		//		{
		//			//Delete Ops Conf
		//			boolean delOpsConf = beanRemote.deleteConfirmations();
		//			if(delOpsConf)
		//				log.info("XXXXXXXXXXXX~~~~~~OPS CONFIRMATIONS DELETED SUCCESSFULLY~~~~~XXXXXXXXXXXX");
		//		}

		//5. Archive Suspensions Responses
		log.info("XXXXXXXXXXXX~~~~~~ARCHIVING MATCHED MANDATE SUSPENSIONS ~~~~~XXXXXXXXXXXX");
		boolean arcSuspBool = beanRemote.archiveSuspensionsBySQL(matchType, sdf.format(responseDate), archDate);
		if(arcSuspBool)
		{
			//Delete Ops Susp
			boolean delOpsSusp = beanRemote.deleteSuspensions(matchType, sdf.format(responseDate));
			if(delOpsSusp)
				log.info("XXXXXXXXXXXX~~~~~~MATCHED SUSPENSIONS DELETED SUCCESSFULLY~~~~~XXXXXXXXXXXX");
		}

		log.info("XXXXXXXXXXXX~~~~~~ARCHIVING EXPIRED MANDATE SUSPENSIONS ~~~~~XXXXXXXXXXXX");
		boolean arcExpSuspBool = beanRemote.archiveSuspensionsBySQL(expireType, sdf.format(responseDate), archDate);
		if(arcExpSuspBool)
		{
			//Delete Ops Susp
			boolean delExpOpsSusp = beanRemote.deleteSuspensions(expireType, sdf.format(responseDate));
			if(delExpOpsSusp)
				log.info("XXXXXXXXXXXX~~~~~~EXPIRED SUSPENSIONS DELETED SUCCESSFULLY~~~~~XXXXXXXXXXXX");
		}

		//5. Archive Mdte Requests
		log.info("XXXXXXXXXXXX~~~~~~ARCHIVING MANDATE REQUEST FOR INFO ~~~~~XXXXXXXXXXXX");
		boolean mdteReqBool = beanRemote.archiveMdteRequestsBySQL(archDate);
		if(mdteReqBool)
		{
			//Delete MdteRequest
			boolean delMdteReq = beanRemote.deleteMdteRequests();
			if(delMdteReq)
				log.info("XXXXXXXXXXXX~~~~~~MANDATE REQUESTS DELETED SUCCESSFULLY~~~~~XXXXXXXXXXXX");
		}
		...
		//2020/08/24: SalehaR - Redundant Method (Flat Table in Use)
		//		log.info("XXXXXXXXXXXX~~~~~~ARCHIVING MANDATE RESP_MSG~~~~~XXXXXXXXXXXX");
		//		boolean mdteRespMsgBool = beanRemote.archiveMdteRespMsgsBySQL(archDate);
		//		if(mdteRespMsgBool)
		//		{
		//			//					//Delete MdteResponseMsg
		//			boolean delMdteRespMsg = beanRemote.deleteMdteRespMsg();
		//			if(delMdteRespMsg)
		//				log.info("XXXXXXXXXXXX~~~~~~MANDATE RESP_MSG DELETED SUCCESSFULLY~~~~~XXXXXXXXXXXX");
		//		}
		...
		if(errCodeBool)
		{
			...
			if(delErrCodes)
				log.info("XXXXXXXXXXXX~~~~~~OPS ERROR CODES DELETED SUCCESSFULLY~~~~~XXXXXXXXXXXX");
		}
		...
		boolean leftOverArc = fileProcessBeanRemote.archiveLeftOverTxnsBySQL(sdf.format(responseDate), archDate);
		if(leftOverArc)
		{
		...
		// Archive Mark Off
		log.info("XXXXXXXXXXXX~~~~~~ARCHIVING Mark Off ~~~~~XXXXXXXXXXXX");
		boolean arcMarkOffBool = beanRemote.archiveMarkOffBySQL();
		if (arcMarkOffBool) {
			// Delete Ops MarkOff
			boolean delOpsMarkOff = beanRemote.deleteMarkOff();
			if (delOpsMarkOff)
				log.info("XXXXXXXXXXXX~~~~~~OPS MarkOff DELETED SUCCESSFULLY~~~~~XXXXXXXXXXXX");
		}
		...
		if(txnsBillDataBool)
		{
			...
			if(delTxnsBillDataBool)
				log.info("XXXXXXXXXXXX~~~~~~DELETING OPS TXNS BILL REPORT DATA SUCCESSFULLY~~~~~XXXXXXXXXXXX");
		}
	}
